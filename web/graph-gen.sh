...
"
   ...
   WARNINGLEADERBOARDURL="http://iabak.archiveteam.org/stats/$SHARD.expireleaderboard-raw"
   ...
   cat $IABAK/web/graph.template.tail | \
	   sed "s/SHARDNAME/${SHARDNAME}/g" | \
	   sed "s!COLLDESC!${COLLDESC}!g" | \
	   sed "s!COLLECTIONS!${COLLECTIONS}!g" | \
	   perl -pe "s!CONNECTIONSGRAPHURL!${CONNECTIONSGRAPHURL}!g" | \
	   perl -pe "s!PROGRESS_GRAPHURL!${PROGRESS_GRAPHURL}!g" | \
	   sed "s/SIZE/${SIZE}/g" | \
	   sed "s/IA1/${IA1}/g" | \
	   sed "s/IA2/${IA2}/g" | \
	   sed "s/IA3/${IA3}/g" | \
	   sed "s/IA4/${IA4}/g" | \
	   sed "s/TIME/${CHRONOS}/g" | \
	   sed "s/CLICOUNT/${CLICOUNT}/g" | \
	   sed "s/CLAMBAKE/${COUNTRYS}/g" | \
	   sed "s!WARNINGLEADERBOARDURL!${WARNINGLEADERBOARDURL}!" | \
	   sed "s/WARNINGLEADERBOARD/${WARNINGLEADERBOARD}/" | \
	   sed "s!EXPIRELEADERBOARDURL!${EXPIRELEADERBOARDURL}!" | \
	   sed "s/EXPIRELEADERBOARD/${EXPIRELEADERBOARD}/" | \
	   sed "s!LEADERBOARDURL!${LEADERBOARDURL}!" | \
	   sed "s/LEADERBOARD/${LEADERBOARD}/" | \
	   sed "s!MAINTSHARDS!${MAINTSHARDS}!" | \
	   sed "s!ACTIVESHARDS!${ACTIVESHARDS}!" >> "$HTMLTMP"
